...
namespace _05_DirectoryTraversal
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                foreach (KeyValuePair<string, List<FileInfo>> kvp in
                    fileByExtension.OrderByDescending(x => x.Value.Count)
                        .ThenBy(x => x.Key))
                
                {
                    writer.WriteLine(kvp.Key);
                    foreach (var fileInfo in kvp.Value.OrderByDescending(x => Math.Ceiling((double)
                        x.Length / 1024)))
                    {
